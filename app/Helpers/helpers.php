...
use App\Services\WebToMiniAppUrlService;
use App\Services\WebToMobileAppUrlService;
use GuzzleHttp\Exception\BadResponseException;
use App\Services\Cloudflare\CloudflareImageService;
...
if (!function_exists('format_phone_number')) {
    function format_phone_number($mobile)
    {
        if (substr($mobile, 0, 3) === "959") {
            $formattedPhoneNumber = "09" . substr($mobile, 3);
        } else {
            $formattedPhoneNumber = $mobile;
        }
...
if (!function_exists('api_response')) {
    ...
    function api_response($data = array(), $status = 200)
    {
        ...
        return response()->json($data, $status, array('Access-Control-Allow-Origin' => '*'));
    }
}